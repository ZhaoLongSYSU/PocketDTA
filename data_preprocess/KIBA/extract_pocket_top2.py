"""
Pocket Extraction - Top 2 Predictions

This script extracts binding pocket residue indices and sequences from 
the combined top 1 and top 2 predicted binding site PDB files.

Input:
    - KIBA_Protein_Domain_Alphfold3d_dict.pickle
    - PDB files in Domain_Key_Pocket_files_top1/ and Domain_Key_Pocket_files_top2/

Output:
    - KIBA_protein_domain_pocket_top2index_dict.pickle
    - KIBA_protein_domain_pocket_top2len_dict.pickle
    - KIBA_protein_domain_pocket_top2seq_dict.pickle
"""
...
PROTEIN_3D_DICT = 'KIBA_Protein_Domain_Alphfold3d_dict.pickle'
...
def pdb_res_index(pdb_file_top1, pdb_file_top2):
    """
    Extract residue indices from two PDB files (top1 and top2 predictions).
    ...
    Returns:
        list: Sorted list of unique residue numbers from both files
    """
    resns = []
...
for key in tqdm(target_key, desc="Extracting pockets"):
    ...
    if not os.path.exists(pdb_file_top1) or not os.path.exists(pdb_file_top2):
        print(f"Warning: PDB file not found for {key}")
        continue
    
    resns = pdb_res_index(pdb_file_top1, pdb_file_top2)
...
with open('KIBA_protein_domain_pocket_top2index_dict.pickle', 'wb') as f:
    pickle.dump(proteins_pocket_index_dict, f)

with open('KIBA_protein_domain_pocket_top2len_dict.pickle', 'wb') as f:
    pickle.dump(proteins_pocket_indexlen_dict, f)

with open('KIBA_protein_domain_pocket_top2seq_dict.pickle', 'wb') as f:
